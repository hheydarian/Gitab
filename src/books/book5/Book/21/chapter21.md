...
* ایجاد جدول‌های ساده با `column` و `printf`.
* استفاده از `printf` در شل و در برنامه‌های دیگر.
* ابزارهای قالب‌بندی مانند `expand`، `fmt` و `pr`.
* تبدیل داده‌ها با `awk` و `sed` برای تولید خروجی ساخت‌یافته.
...
* مستند `info printf` در پروژهٔ GNU جزئیات همهٔ توصیفگرهای قالب را فهرست می‌کند.
* برای ایده‌های بیشتر دربارهٔ گزارش‌سازی با `awk`، به کتاب «The AWK Programming Language» مراجعه کنید.
* صفحهٔ راهنمای `man column` و `man pr` مثال‌های بیشتری دربارهٔ قالب‌بندی خروجی ارائه می‌دهند.
